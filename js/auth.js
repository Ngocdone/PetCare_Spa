...
<<<<<<< HEAD
    ...
    .then(data => {
      ...
      if (data.user && data.user.role === 'admin') {
        setCurrentUser({ id: data.user.id, name: data.user.name, email: data.user.email, role: 'admin' });
        window.location.href = 'admin/index.html';
      } else {
        setCurrentUser({ id: data.user.id, name: data.user.name, email: data.user.email, role: data.user.role || 'user' });
    ...
    .catch(err => {
      ...
      // Fallback to local storage login if server fails
