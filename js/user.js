...
function getAuthToken() {
  return localStorage.getItem('petspa_token');
}
...
async function fetchPetsFromAPI() {
  try {
    ...
    const response = await fetch(`${API_BASE}/pets/my`, {
...
// Update pet in API
async function updatePetAPI(id, petData) {
  try {
    const token = getAuthToken();
    ...
    if (response.ok) {
      ...
      if (idx >= 0) petsCache[idx] = updatedPet;
      return updatedPet;
    }
    return null;
  } catch (e) {
...
// Delete pet from API
async function deletePetAPI(id) {
  try {
    const token = getAuthToken();
    const response = await fetch(`${API_BASE}/pets/${id}`, {
      method: 'DELETE',
  ...
  function init() {
    ...
    document.getElementById('formPet')?.addEventListener('submit', async function (e) {
      ...
      const petData = {
        ...
        type: document.getElementById('petType').value || 'cho',
      ...
      if (id) {
        // Update existing pet
        success = await updatePetAPI(id, petData);
      } else {
        // Create new pet
        success = await savePetToAPI(petData);
      }
